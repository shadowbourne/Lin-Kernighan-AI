...
max_change = 0
start = time.time()
while True and start - time.time() < 105:
    for i in range(num_cities-2):
        for j in range(i+1,num_cities-1):
            change = distance_matrix[tour[i]][tour[i+1]] + distance_matrix[tour[j]][tour[j+1]] - distance_matrix[tour[i]][tour[j]] - distance_matrix[tour[i+1]][tour[j+1]]
